...
class Trainset:
    ...
    def __init__(self, ur, ir, n_users, n_items, n_ratings, rating_scale,
        ...
        # print("raw2inner users length %d" % len(raw2inner_id_users))
        self._raw2inner_id_items = raw2inner_id_items
        # print("raw2inner items length %d" % len(raw2inner_id_items))


        self._inner2raw_id_users = {v : k for k, v in raw2inner_id_users.iteritems()}
        self._inner2raw_id_items = {v : k for k, v in raw2inner_id_items.iteritems()}

        # print("inner2raw users length %d" % len(self._inner2raw_id_users))
        # print("raw2inner items length %d" % len(self._inner2raw_id_items))
    ...
    def to_raw_uid(self, iuid):
        try:
            ...
            raise ValueError(('User ' + str(iuid) +
                              ' has No Raw ID'))
    ...
    def to_raw_iid(self, iiid):
        try:
            ...
            raise ValueError(('Item ' + str(iiid) + ' has No Raw Id'))
